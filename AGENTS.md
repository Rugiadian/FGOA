...
- **무관한 파일/캐시 접근 금지**: `.geminiignore` 및 `.gitignore`에 등록된 항목(`history/`, `references/`, `__pycache__/`, `*.log`, 대용량 JSON, 이미지 파일 등)을 불필요하게 AI 컨텍스트에 주입하거나 전역 검색하지 않습니다.
- **최소 범위 파일 조회 및 편집**: 대형 소스 코드(800줄 초과, 특히 `ui/main_window.py`, `ui/inspector_widget.py`) 탐색 시 파일 전체를 반복 로드하지 않고, 검색 명령어로 대상 위치를 파악한 뒤 `StartLine`/`EndLine`을 30~60줄 이내로 최소화하여 조회 및 수정합니다.
- **테스트 실행 시 표준 출력 버퍼링 필수 (`-b`)**: 전체 테스트 스위트 실행 시 반드시 `python -m unittest discover -b -s tests`를 사용하여 통과된 테스트의 불필요한 표준 출력(stdout/print문)이 AI 컨텍스트에 대량 누적되는 것을 방지합니다. 또한 단위 테스트 내에 불필요한 장문의 콘솔 `print()` 구문을 작성하지 않습니다.
